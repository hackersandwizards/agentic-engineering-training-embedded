...
void CommandHandler::handle(const Frame& request, const Interlocks::Inputs& state) {
    ...
    if (request.msg_id == MsgId::Ping) {
        respond(request, nullptr, 0);
    } else if (request.msg_id == MsgId::GetVersion) {
        const std::uint8_t version[] = {0, 9, 0};
        respond(request, version, sizeof(version));
    } else if (request.msg_id == MsgId::MotorSetTarget) {
    ...
    } else if (request.msg_id == MsgId::MotorStop) {
        motor_->stop();
        respond(request, nullptr, 0);
    } else if (request.msg_id == MsgId::HeaterSetTarget) {
        ...
        respond(request, nullptr, 0);
    } else if (request.msg_id == MsgId::HeaterOff) {
        pid_->disable();
        respond(request, nullptr, 0);
    } else if (request.msg_id == MsgId::ScaleTare) {
        const auto raw = scale_->read_raw();
        if (!raw.is_ok()) {
            nack(request);
            return;
